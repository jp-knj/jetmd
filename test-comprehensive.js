  ...
  const tests = [
    ...
    },
    {
      ...
      checkAst: (ast) => {
        ...
        return p.children.some(c => c.type === 'strong') && p.children.some(c => c.type === 'emphasis');
      }
    },
    {
      name: 'Code block',
...
[Learn more](https://example.com)`,
  ...
  for (const test of tests) {
    try {
      ...
      console.log(`  Parse time: ${ast.parseTime ? (ast.parseTime / 1000000).toFixed(2) + 'ms' : 'N/A'}`);
  ...
  console.log(`Throughput: ${((perfDoc.length * iterations) / (elapsed / 1000) / 1024 / 1024).toFixed(2)} MB/s`);
