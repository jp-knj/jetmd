...
async function detailedBenchmark() {
  ...
  for (const r of results) {
    console.log(
      `| ${r.size} | ${r.median.toFixed(2)}ms | ${r.p95.toFixed(2)}ms | ${r.p99.toFixed(
        2,
      )}ms | ${r.throughput.toFixed(1)} MB/s |`,
    )
  }
  ...
  if (target50KB) {
    ...
    console.log(
      `  実績: ${target50KB.median.toFixed(2)}ms (P50)、${target50KB.p99.toFixed(2)}ms (P99)`,
    )
