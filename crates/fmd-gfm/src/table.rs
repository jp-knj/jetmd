...
impl TableParser {
    ...
    pub fn parse_table(&self, lines: &[String], start_line: usize) -> Option<Node> {
        ...
        let header_row = Node {
            ...
            children: header_cells.into_iter().enumerate().map(|(i, content)| {
                self.create_cell(content, true, alignments.get(i).copied())
            }).collect(),
            ..Default::default()
        };
        ...
        for line in &lines[2..] {
            ...
            if let Some(cells) = self.parse_row(line) {
                let row = Node {
                    node_type: NodeType::TableRow,
                    children: cells.into_iter().enumerate().map(|(i, content)| {
    ...
    fn create_cell(&self, content: String, is_header: bool, alignment: Option<Alignment>) -> Node {
        ...
        if is_header {
            node.data.insert("header".to_string(), serde_json::Value::Bool(true));
        }
        ...
        if let Some(align) = alignment {
            ...
            node.data.insert("align".to_string(), serde_json::Value::String(align_str.to_string()));
        }
